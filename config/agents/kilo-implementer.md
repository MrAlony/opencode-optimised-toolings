...
- Use `alonix-web-search` for batched discovery, `alonix-web-fetch-many` for 1–10 known URLs with safe extraction and adaptive output, and Alonix stealth tools only when normal retrieval is blocked, requires JavaScript rendering, or has a concrete Tor/privacy requirement. Prefer `alonix-web-search` fallback strategy; use aggregate deliberately for breadth/corroboration. Set `allow_private=true` only for intentional local-service fetching.
