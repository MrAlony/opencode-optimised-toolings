...
try {
  const editOutput = await check("fs_edit_many", () => plugin.tool.fs_edit_many.execute({
    base_dir: fixture,
    actions: [
  ...
  await check("fs_read_many", () => plugin.tool.fs_read_many.execute({
    base_dir: fixture,
  ...
  const backgroundStart = await check("background_process", () => plugin.tool.background_process.execute({
  ...
  if (backgroundId) {
    ...
    const lifecycle = await plugin.tool.background_process.execute({ operations: [{ action: "status", id: backgroundId }, { action: "logs", id: backgroundId }, { action: "stop", id: backgroundId }, { action: "cleanup" }] }, context("live-background-lifecycle"));
    const record = results.find((item) => item.name === "background_process");
  ...
  } else results.find((item) => item.name === "background_process").passed = false;
  ...
  await check("cbm_context", () => plugin.tool.cbm_context.execute({ project: cbmProject }, context("live-cbm-context")), (output) => /INDEX READINESS/.test(output) && /ARCHITECTURE/.test(output) && !/^CBM CONTEXT FAILED/m.test(output));
  await check("cbm_investigate", () => plugin.tool.cbm_investigate.execute({
    project: cbmProject,
  ...
  await check("cbm_memory", () => plugin.tool.cbm_memory.execute({
  ...
  await check("web_fetch_many", () => plugin.tool.web_fetch_many.execute({
  ...
  await check("web_search", () => plugin.tool.web_search.execute({
  ...
  await check("stealth_status", () => plugin.tool.stealth_status.execute({}, context("live-stealth-status")), (output) => /STEALTH STATUS: READY/.test(output) && /Python worker: running/.test(output));
  await check("stealth_fetch_many", () => plugin.tool.stealth_fetch_many.execute({
  ...
  await check("stealth_search_many", () => plugin.tool.stealth_search_many.execute({
