...
test("shell executes independent commands in one parallel batch", async () => {
  const output = await plugin.tool.shell.execute({
    commands: [
...
test("shell safely repairs a quoted Windows executable invocation inside one call", { skip: process.platform !== "win32" }, async () => {
  const executable = process.execPath.replaceAll('"', '""');
  const output = await plugin.tool.shell.execute({ commands: [{ command: `"${executable}" -e "console.log('recovered-shell')"` }] }, { ...context, sessionID: "quoted-executable-recovery" });
  assert.match(output, /TERMINAL RESULT: SUCCESS/);
...
test("singleton shell calls emit escalating advice", async () => {
  ...
  const first = await plugin.tool.shell.execute(args, singletonContext);
  const second = await plugin.tool.shell.execute(args, singletonContext);
...
test("shell flags filesystem read and search substitutions even in a batch", async () => {
  ...
  assert.match(output, /fs_read_many/);
  assert.match(output, /fs_search/);
});
...
test("shell enforces timeout and kills the process tree", async () => {
  ...
  assert.doesNotMatch(output, /use background_process/i);
});
...
test("repeated pure polls emit escalating advice without delaying operations", async () => {
  ...
  const first = await plugin.tool.background_process.execute(args, pollContext);
  const started = Date.now();
  const second = await plugin.tool.background_process.execute(args, pollContext);
  const third = await plugin.tool.background_process.execute(args, pollContext);
...
test("non-polling background work resets the poll advisory streak", async () => {
  ...
  await plugin.tool.background_process.execute(poll, resetContext);
  await plugin.tool.background_process.execute({ operations: [{ action: "cleanup" }] }, resetContext);
  const nextPoll = await plugin.tool.background_process.execute(poll, resetContext);
...
test("another tool invocation resets the background polling advisory streak", async () => {
  ...
  const poll = { operations: [{ action: "status", id: "missing" }] };
  await plugin.tool.background_process.execute(poll, resetContext);
  await plugin["tool.execute.before"]({ tool: "fs_read_many", sessionID: resetContext.sessionID });
  const nextPoll = await plugin.tool.background_process.execute(poll, resetContext);
  assert.doesNotMatch(nextPoll, /POLLING ADVICE|CRITICAL POLLING ADVICE/);
});

test("background_process batches starts, listing, and stop_all", async () => {
  const output = await plugin.tool.background_process.execute({
    operations: [
...
test("background start returns readiness and startup output in the same operation", async () => {
  const output = await plugin.tool.background_process.execute({ operations: [{ action: "start", command: "node -e \"console.log('READY-LIVE');setTimeout(() => {}, 30000)\"", ready_output: "READY-LIVE", startup_timeout_ms: 3_000 }, { action: "stop_all" }, { action: "cleanup" }] }, { ...context, sessionID: "background-readiness-test" });
...
test("background start reports an early exit and logs without a follow-up call", async () => {
  const output = await plugin.tool.background_process.execute({ operations: [{ action: "start", command: "node -e \"console.error('startup-boom');process.exit(12)\"" }] }, { ...context, sessionID: "background-early-exit-test" });
...
test("background safely repairs a quoted Windows executable during startup", { skip: process.platform !== "win32" }, async () => {
  const executable = process.execPath.replaceAll('"', '""');
  const output = await plugin.tool.background_process.execute({ operations: [{ action: "start", command: `"${executable}" -e "console.log('BG-READY');setTimeout(() => {}, 30000)"`, ready_output: "BG-READY", startup_timeout_ms: 3_000 }, { action: "stop_all" }, { action: "cleanup" }] }, { ...context, sessionID: "background-recovery-test" });
  assert.match(output, /PROCESS READY/);
  assert.match(output, /AUTOMATIC RECOVERY/);
  assert.match(output, /Outcome: SUCCEEDED/);
  assert.match(output, /Stop completed for 1 of 1/);
...
test("plugin disposal terminates all managed background processes", async () => {
  ...
  await disposablePlugin.tool.background_process.execute({
    operations: [{ action: "start", command: "node -e \"setTimeout(() => {}, 30000)\"" }],
  }, { ...context, sessionID: "dispose-test" });
  await disposablePlugin.dispose();
  const output = await disposablePlugin.tool.background_process.execute({
...
test("background_process tracks and stops a detached Windows child", { skip: process.platform !== "win32" }, async () => {
  const backgroundContext = { ...context, sessionID: "detached-background-test" };
  const started = await plugin.tool.background_process.execute({
  ...
  await delay(1_500);
  const status = await plugin.tool.background_process.execute({ operations: [{ action: "status", id }] }, backgroundContext);
  ...
  await plugin.tool.background_process.execute({ operations: [{ action: "stop", id }] }, backgroundContext);
  await delay(500);
