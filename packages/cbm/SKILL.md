...
2. If the repository is absent, call `cbm_project` with `action="index"`, its absolute `repo_path`, and normally `mode="fast"`.
3. Call `cbm_context` once for architecture, graph schema, and change blast radius.
...
Grouped index management: `list`, `index`, `status`, and `delete`. Indexing is explicit and bounded. Never start duplicate indexing.
...
- On first use after installing this version, unknown-baseline indexes self-refresh once. Later stale or unverified indexes also self-refresh before queries; concurrent callers share the same bounded refresh.
