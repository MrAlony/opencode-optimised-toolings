...
test("settings is a first-class mouse-accessible route", async () => {
  ...
  assert.match(entry, /pluginVersion=\{attestation\.version \?\? "unknown"\}/)
  assert.match(settings, /Loaded plugin version/)
  assert.match(settings, /v\{props\.pluginVersion \?\? "unknown"\}/)
...
test("activation transaction supports one package declaration with the TUI entry kept internal", async () => {
  ...
  assert.match(generation, /tuiCoordinationPath/)
  assert.match(generation, /authority: "opencode-optimised-toolings-control-plane"/)
  ...
  assert.match(generation, /if \(!item\.existed\) await fs\.rm\(item\.file[\s\S]*?else await atomicWrite\(item\.file, item\.before\)/, "rollback restores existing outputs and removes outputs created by the interrupted transaction")
