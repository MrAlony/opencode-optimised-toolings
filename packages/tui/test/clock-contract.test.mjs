...
test("all persisted IDE state waits for the host KV readiness signal", async () => {
  ...
  assert.match(entry, /await projects\.waitForInitialLoad\(\)/)
  assert.ok(entry.indexOf("await projects.waitForInitialLoad()") < entry.indexOf("api.slots.register"), "IDE slots must not become visible before portfolio startup settles")
