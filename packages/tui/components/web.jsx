...
export function WebView(props) {
  ...
  const summary = createMemo(() => parsed() ? `${counts().succeeded ?? visible().succeeded}/${batch().plannedCount} ${isFetch() ? "URLs" : "queries"} succeeded${batch().omitted.length ? ` · ${batch().omitted.length} detail${batch().omitted.length === 1 ? "" : "s"} omitted` : ""}` : pendingPlanSummary(planReady(), batch().plannedCount, noun(), isFetch() ? "URLs" : "queries"))
  const details = () => {
    const result = parsed()
    ...
    if (statusPending(status())) return <InspectorCard title={planReady() ? `${label()} plan` : `Preparing ${label().toLowerCase()}`} skin={props.skin} status={status()} pending meta={planReady() ? `${batch().plannedCount} item(s)` : "input pending"}>{planReady() ? <PreviewList skin={props.skin} items={items()} limit={12} /> : <text fg={props.skin.muted}>Waiting for OpenCode to attach the validated request input.</text>}</InspectorCard>
