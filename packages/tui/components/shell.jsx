...
export function ShellView(props) {
  ...
  const summary = createMemo(() => resultStatus() ? `${counts().succeeded ?? visible().succeeded}/${batch().plannedCount} commands succeeded${batch().omitted.length ? ` · ${batch().omitted.length} details omitted` : ""}` : `${batch().plannedCount} command${batch().plannedCount === 1 ? "" : "s"}`)
  const details = () => {
    ...
    if (statusPending(status())) return <InspectorCard title="Command plan" skin={props.skin} status={status()} pending meta={`${batch().plannedCount} command(s)`}><PreviewList skin={props.skin} items={items()} limit={12} /></InspectorCard>
  ...
  return <Activity label="Shell" summary={summary()} meta={statusLabel(status())} status={status()} pending={statusPending(status())} skin={props.skin} preview={<PreviewList skin={props.skin} items={items()} />} details={details} />
}
