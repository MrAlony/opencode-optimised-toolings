...
export function BackgroundView(props) {
  ...
  const summary = createMemo(() => operations().length ? `${batch().plannedCount} operation${batch().plannedCount === 1 ? "" : "s"} · ${operations().filter((op) => operationStatus(op) === "FAILED").length} failed${batch().omitted.length ? ` · ${batch().omitted.length} details omitted` : ""}` : `${batch().plannedCount} operation${batch().plannedCount === 1 ? "" : "s"}`)
  const details = () => {
    ...
    if (statusPending(status())) return <InspectorCard title="Process plan" skin={props.skin} status={status()} pending meta={`${batch().plannedCount} operation(s)`}><PreviewList skin={props.skin} items={items()} limit={12} /></InspectorCard>
