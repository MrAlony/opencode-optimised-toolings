...
export function inputPlanAvailable(tool, input) {
  ...
  if (tool === "alonix-web-fetch" || tool === "alonix-stealth-fetch") return Array.isArray(input?.requests) && input.requests.length > 0
  if (tool === "alonix-web-search" || tool === "alonix-stealth-search") return Array.isArray(input?.queries) && input.queries.length > 0
  if (tool === "alonix-read") return (input?.paths?.length ?? 0) + (input?.requests?.length ?? 0) > 0
  if (tool === "alonix-edit") return Array.isArray(input?.actions) && input.actions.length > 0
